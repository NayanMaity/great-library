    ...
    if (empty($name) || empty($email)) {
        ...
        if ($email == $user_data['user_email']) {
            ...
            if (count($err_msg) == 0) {
                ...
                if (!empty($_FILES['admin-update-avatar']['name'])) {

                    if ($avatar !== null) {
                        unlink("../uploads/" . $avatar);
                    }

                    $avatar = time() . "_" . str_replace(" ", "_", $name) . "_" . $_FILES['admin-update-avatar']['name'];
                    move_uploaded_file($_FILES['admin-update-avatar']['tmp_name'], "../uploads/" . $avatar);
                }
