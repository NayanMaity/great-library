    ...
    if (empty($name) || empty($email) || empty($pass)) {
        ...
        if (count($err_msg) == 0) {
            ...
            if (count($resSearch) > 0) {
                ...
                if (!empty($_FILES['std-reg-avatar']['name'])) {

                    $avatar = time() . "_" . str_replace(" ", "_", $name) . "_" . $_FILES['std-reg-avatar']['name'];
                    move_uploaded_file($_FILES['std-reg-avatar']['tmp_name'], "../uploads/" . $avatar);
                }
